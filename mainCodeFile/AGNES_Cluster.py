...
def drawnode(draw,clust,x,y,scaling,labels):
    if clust.id<0:
        ...
        draw.text((x+5,y-7),labels[clust.id],(0,0,0),font=ImageFont.truetype('simhei.ttf', 15))
...
def whole_Cluster_process(csvFile='embedding.csv',jpgSave='clustrRes.jpg'):
    startT = time.time()
    print('\n开始进行聚类： ')
    blognames,words,data = readfile(csvFile)  #加载数据集
    clust = hcluster(data)  #构建聚类树
    drawdendrogram(clust,blognames,jpeg=jpgSave)  # 绘制聚类树

    endT = time.time()
    print('聚类时间消耗： {}s'.format(endT-startT))

if __name__=='__main__':
    whole_Cluster_process()
